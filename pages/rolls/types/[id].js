...
export async function getStaticPaths() {
  const data = (await axios.get(`${process.env.DB_HOST}/rolls/api/rolltype`)).data
...
export async function getStaticProps({params}){
  console.log(params)
  const roll_type = (await axios.get(`${process.env.DB_HOST}/rolls/api/rolltype/${params.id}`)).data
